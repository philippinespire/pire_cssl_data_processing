...
# Assessing missing files
ls *zip > zipfiles
ls *fq.gz > fqfiles
sed -i 's/_fastqc\.zip//' zipfiles
sed -i 's/\.fq\.gz//' fqfiles
grep -v -f zipfiles fqfiles > missingfiles
sed -i 's/$/\.fq\.gz/' missingfiles

#run fastqc in parallel 
#ls ${inDIR}/*${PATTERN} | parallel --no-notice -j32 "crun fastqc {}"
cat missingfiles | parallel --no-notice -j40 "crun fastqc {}"
