...
# This script concatenates files from the same individual but from different lane (L1, L2, L3, L4, etc)
...
# Concatenate files
ls *gz | sed 's/_L.*//' | uniq | parallel --no-notice -kj20 "cat {}*1.fq.gz > $OUTDIR/{}_LA_1.fq.gz"
ls *gz | sed 's/_L.*//' | uniq | parallel --no-notice -kj20 "cat {}*2.fq.gz > $OUTDIR/{}_LA_2.fq.gz"
