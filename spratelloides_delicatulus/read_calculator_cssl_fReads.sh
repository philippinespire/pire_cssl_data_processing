...
#######  read_calculator_cssl.sh  ########

#read_calculator_cssl.sh counts the number of reads before and after each step in the pre-process of cssl data and creates 2 tables reporting
...
# Create and move to preprocess_read_change_fReads directory
mkdir -p ${SPDIR}/preprocess_read_change_fReads

echo "read_calculator_cssl.sh counts the number of reads before and after each step in the pre-process of cssl data and creates 2 tables reporting
  (1) "readLoss_table.tsv" the step-specific percent of read loss and final accumulative read loss
...
ls ${RAWDIR}/*1.fqgz | parallel --no-notice -kj${PARALLELISM} "echo -n {}'	' && zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/raw.temp
ls ${SPDIR}/fq_fp1/*1.fq.gz | parallel --no-notice -kj${PARALLELISM} "zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/fp1.temp
ls ${SPDIR}/fq_fp1_clmp/*1.fq.gz | parallel --no-notice -kj${PARALLELISM} "zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/clm.temp
ls ${SPDIR}/fq_fp1_clmp_fp2/*1.fq.gz | parallel --no-notice -kj${PARALLELISM} "zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/fp2.temp
ls ${SPDIR}/fq_fp1_clmp_fp2_fqscrn/*1.tagged_filter.fastq.gz | parallel --no-notice -kj${PARALLELISM} "zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/fqscrn.temp
ls ${SPDIR}/fq_fp1_clmp_fp2_fqscrn_rprd/*1.fq.gz | parallel --no-notice -kj${PARALLELISM} "zgrep '^@' {} | wc -l" > ${SPDIR}/preprocess_read_change_fReads/repr.temp

cd ${SPDIR}/preprocess_read_change_fReads
