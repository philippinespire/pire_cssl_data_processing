...
#ls *fq.gz > filesall
#ls *html | sed 's/_fastqc\.html/\.fq\.gz/' > filesdone

#run fastqc in parallel 
#grep -vf filesdone filesall | xargs ls | xargs crun fastqc &&

#rm filesdone filesall

# run multiqc with specific report and subdirectory names
crun multiqc . -n multiqc_report_$1.html -o .

# move fastqc files to new subdirectory
#ls *fastqc.html | parallel -j 40 "mv {} ../Multi_FASTQC" &&
#ls *fastqc.zip | parallel -j 40 "mv {} ../Multi_FASTQC"
#mv *out ../logs
